$(function(){
	$.ajax({
		...
		success: function(data){
			...
			for(var i in data.vueJs){
				$("#skill_int1 ul").append("<li>"+data.vueJs[i]+"</li>");
			}
    ...
    $('#dowebok').fullpage({
		...
		afterLoad: function(anchorLink,index){
			...
			if(index==4){
				...
				$("#exp_content h1").after("<div class='title_en'><h2>· Practice ·</h2></div>");
				$(".title_en").animate({width:"130px"},800,function(){
					$(".title_en h2").slideDown(400);
				});
                ...
                function run(){
                    timer = setInterval(function(){
                        $("#exp_list_toright").trigger("click");
                    }, 1000);
                }
                run();
			...
			if(index==5){
				...
				$("#demo_content h1").after("<div class='title_en'><h2>· Study·</h2></div>");
				$(".title_en").animate({width:"130px"},800,function(){
					$(".title_en h2").slideDown(400);
				});	
				var i=-1;
			...
			if(index==6){
				...
				var i=-1;
				$("#contact_head1").addClass("b_to_t").addClass("animated").addClass("rubberBand");
...
// 图片轮播
	$("#exp_list_slider").width($(".exp_list").width());
     $("#exp_list_content").children(":first").clone(true).appendTo("#exp_list_content");
     var len = $(".exp_list").length;
    $("#exp_list_content").width($(".exp_list").width()*len);
	$("#exp_list_slider_content").mouseenter(function(){
	...
	$("#exp_list_toleft").click(function() {
		...
		if(!$("#exp_list_content").is(":animated")){
                if(page==-1){
                	page=len-2;
                    $("#exp_list_content").css({left:-$(".exp_list").width()*(len-1)});
                }
            $("#exp_list_content").animate({left:(-$(".exp_list").width()*page)},2000);
			}
	});
